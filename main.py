...
request = Debugger.input("Encrypt or decrypt (e/d) >> ").strip().lower()
if request=="e":
    key = Debugger.input("Set the key string here (press return to generate a random one) >> ").strip()
    if not key:
        key = ''.join(random.choices(string.ascii_letters + string.digits, k=40))
        Debugger.info("The randomly generated key is :" + key + " , make sure to keep it")
elif request=="d":
    key = Debugger.input("Set the key string here >> ").strip()

allFiles = Debugger.input("Apply to all files in folder? (y/n) >> ").strip().lower()
...
def encryptFile(file):
    ...
    Debugger.info(f"Encrypted file saved as: {outputFile}")
...
def decryptFile(file):
    ...
    if magicBytes == b"SUCCESS":
        Debugger.error("This file has been encrypted using cws-v1, please use the appropriate decryptor.")
        return
    if not magicBytes == b"\x69\x74crypt":
        Debugger.error("This file is not a cws-encrypted file!")
        return
    fileExt = stream.readString()
    fileVersion = stream.readVInt()
    if fileVersion != 2:
        Debugger.error("Incorrect cws version")
        return
    ...
    if not passwordCheck == b"cws":
        Debugger.error("Incorrect decryption password!")
        return
    outputFile = file.replace(".cws", f".{fileExt}")
    with open(outputFile, "wb") as decryptedFile:
        decryptedFile.write(decryptedData)
    Debugger.info(f"Decrypted file saved as: {outputFile}")
...
if allFiles == "y":
    for f in os.listdir('.'):
        if request == "e" and os.path.isfile(f) and not f.endswith(".cws"):
            ...
            Debugger.warning("Invalid choice! Please enter 'e' to encrypt or 'd' to decrypt.")
else:
    file = Debugger.input("File name >> ").strip()
    if request == "e":
        encryptFile(file)
    elif request == "d":
        decryptFile(file)
